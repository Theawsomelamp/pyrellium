...
public class GeodinEntity extends AnimalEntity {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(1, new TemptGoal(this, 1.25D, stack -> stack.isIn(TagKey.of(RegistryKeys.ITEM, Identifier.of(Pyrellium.MOD_ID, "crystals"))), false));
    ...
    public static DefaultAttributeContainer.Builder createGeodinAttributes() {
        return AnimalEntity.createAnimalAttributes()
                .add(EntityAttributes.MAX_HEALTH, com.lankaster.pyrellium.config.Config.instance().entities.geodin.attributes.max_health())
                .add(EntityAttributes.MOVEMENT_SPEED, com.lankaster.pyrellium.config.Config.instance().entities.geodin.attributes.movement_speed())
                .add(EntityAttributes.ARMOR, com.lankaster.pyrellium.config.Config.instance().entities.geodin.attributes.armor());
    }
    ...
    public void writeCustomData(WriteView view) {
        ...
        view.putInt("CrystalAge", getCrystalAge());
        view.putString("Variant", this.getVariant().id.toString());
    }

    @Override
    public void readCustomData(ReadView view) {
        super.readCustomData(view);
        this.setCrystalAge(view.getInt("CrystalAge", 0));
        this.setVariant(Variant.get(Identifier.of(view.getString("Variant", "minecraft:amethyst"))));
    }
    ...
    public @Nullable PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        return ModEntities.GEODIN.create(world, SpawnReason.BREEDING);
    }
    ...
    protected void mobTick(ServerWorld world) {
        ...
        if (this.stuckTicks % 100 == 0) {
            ...
            if (this.stuckTicks >= com.lankaster.pyrellium.config.Config.instance().entities.geodin.conversion_time && !isBlockAtPosSolid(this.getBlockPos(), world)) {
                world.setBlockState(this.getBlockPos(), getBlockForm());
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        if (player.getStackInHand(hand).isIn(ItemTags.PICKAXES)) {
            if (getCrystalAge() >= 1) {
                Block.dropStacks(getVariant().getBlockFromAge(getCrystalAge()).getDefaultState(), player.getEntityWorld(), this.getBlockPos(), null, null, player.getStackInHand(hand));
                player.getEntityWorld().playSound(null, this.getBlockPos(), SoundEvents.BLOCK_AMETHYST_BLOCK_BREAK, SoundCategory.BLOCKS);
    ...
    private boolean canTravel(ServerWorld world) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (isBlockAtPosSolid(blockPos, world)) {
                return false;
            }
        }
    ...
    private boolean isBlockAtPosSolid(BlockPos pos, ServerWorld world) {
        return world.getBlockState(pos).hasSolidTopSurface(world.getChunkAsView(pos.getX(), pos.getY()), pos, this);
    }
